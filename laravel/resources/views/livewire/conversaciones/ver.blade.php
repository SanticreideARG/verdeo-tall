...
<div x-data="{ flash: false }" @guardado.window="flash = true; setTimeout(() => flash = false, 3000)">
    ...
    <div class="flex items-center justify-between mb-6">
        ...
        @if($conversacion->canalUrl())
        <a href="{{ $conversacion->canalUrl() }}" target="_blank"
           class="btn-secondary text-sm flex items-center gap-2">
            @if(($conversacion->canal ?? 'whatsapp') === 'whatsapp')
            ...
            Abrir WhatsApp
            @elseif($conversacion->canal === 'messenger')
            <svg width="15" height="15" viewBox="0 0 24 24" fill="#0078ff">
            ...
            <div class="flex-1 min-w-0">
                <div class="flex items-center gap-3 flex-wrap mb-1">
                    ...
                    <span class="inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full font-semibold"
                          style="background: {{ $conversacion->canalColor() }}18;
                                 border: 1px solid {{ $conversacion->canalColor() }}44;
                                 color: {{ $conversacion->canalColor() }};">
                        @if(($conversacion->canal ?? 'whatsapp') === 'messenger')
                        <svg width="10" height="10" viewBox="0 0 24 24" fill="currentColor">
                            <path d="M12 2C6.36 2 2 6.13 2 11.7c0 2.91 1.19 5.44 3.14 7.17.16.13.26.35.27.56l.05 1.76a.75.75 0 001.05.66l1.96-.87c.17-.07.36-.1.54-.07.9.25 1.9.38 2.99.38 5.64 0 10-4.13 10-9.7S17.64 2 12 2zm6 7.46l-2.93 4.67a1.51 1.51 0 01-2.18.4L10.77 13a.6.6 0 00-.72 0l-2.84 2.16c-.38.29-.88-.17-.63-.58L9.51 9.9a1.51 1.51 0 012.18-.4l2.12 1.53a.6.6 0 00.72 0l2.84-2.16c.38-.29.88.17.63.59z"/>
                        </svg>
                        @elseif($conversacion->canal === 'instagram')
                        <svg width="10" height="10" viewBox="0 0 24 24" fill="currentColor">
                ...
                <div class="flex items-center gap-2 mb-2" x-data="{ copied: false }">
                    <span class="font-mono text-sm" style="color: var(--vd-text-soft);">{{ $conversacion->canalIdentifier() }}</span>
                    <button type="button"
                            @click="navigator.clipboard.writeText('{{ $conversacion->canalIdentifier() }}').then(() => { copied = true; setTimeout(() => copied = false, 2000) })"
